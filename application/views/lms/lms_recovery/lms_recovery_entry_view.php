...
?>
    ...
    <?php 
        $attributesForm = array('name' => 'lmsBasicRecForm', 'id'=>'lms_basic_search_rec_info_form');
        echo form_open_multipart('lms/lms_recovery_entry_details_view', $attributesForm);
    ?>
    <div class="header-info text-center">
       <h4>Search for Recovery</h4>
    </div>
    <table class="search_tbl">
        
        <tr>
            <td><div class="form-group"><label for="tracNo">Trac No.</label></div></td>
            <td>
            ...
            <td>
                <div class="form-group">
                <input type="button" class="btn btnSave" value="Search" <?php echo $attribute; ?> style="background-color: #bb33ff; padding: 0px 18px;" onclick="lms_recovery_view_search(this.value)"/>            
                </div>
            </td>                    
        </tr>
    </table>
    ...
    <table class="table table-bordered" border="1" id="mydata">
        ...
        <?php foreach($lmseditDatas as $lmseditData) {?>
            <tr>
                <td><?php 
                    echo anchor('lms/lms_recovery_entry_details_view/'.$lmseditData->lb_tran_no, $lmseditData->lb_tran_no, 'title="Edit"');
                ?>
                </td>
