...
<p>
    ...
    <?php
    //echo "<pre>";
//print_r($iss_2_abn_incrdecr_data);
//die();
    ?>
  ...
  <?php
   ...
   if(isset($iss_2_abn_incrdecr_data) && count($iss_2_abn_incrdecr_data)>1)
   {
	...
	<?php
		...
		{
			if(isset($iss_2_whole_br_raw) && count($iss_2_whole_br_raw)>0){
...
?>
	...
	<tr>
		...
		<input type="radio" name="amt_fig" id="act_val" onclick="check_fig_indication_iss2_query(this.value)" <?php  echo isset($checked_1) ? $checked_1: ' ';?> value="1"> Actual
  		<input type="radio" name="amt_fig" id="lac_val" onclick="check_fig_indication_iss2_query(this.value)" <?php echo isset($checked_2) ? $checked_2: ' '; ?> value="2"> Lac
  		<input type="radio" name="amt_fig" id="crr_val" onclick="check_fig_indication_iss2_query(this.value)" <?php echo isset($checked_3) ? $checked_3: ' '; ?> value="3"> Crore</td>
	</tr>
    <?php
	...
	if(isset($login_office_status) && $login_office_status != 4)
	{
		...
		echo "<td align='left'>"."<strong>"."Branch"."<strong>"."</td>";
	}
	...
	echo "<td align='center' style='font-size:9px'>"."<strong>"."Diff</br>(Amount in $fig_ind_desc </br> ($report_of_date2) - Amount in $fig_ind_desc </br>($report_of_date1))"."<strong>"."</td>";
	...
	{
		$co=1;
		foreach($iss_2_abn_incrdecr_data as $row1)
		{
			echo "<tr>";
			echo "<td align='right'>".$co."</td>";
			if(isset($login_office_status) && $login_office_status != 4)
			{
				echo "<td align='left'>".$row1->dvname."</td>";
				echo "<td align='left'>".$row1->znname."</td>";
				echo "<td align='left'>".$row1->branchname."</td>";
			}
			...
			if($row1->COA_ID_VALUE==2)
			{
				...
				{
					echo "<td align='right'>".number_format($row1->diff_pre, 0, '.', '')."</td>";
					echo "<td align='right'>".number_format($row1->diff_next, 0, '.', '')."</td>";
				...
				else
				{
					...
					echo "<td align='right'>".number_format($row1->diff_per, 0, '.', '')."</td>";
				}
			...
			else
			{
				...
				{
					$temp_amt1 = $row1->diff_pre/10000000;
					$temp_amt2 = $row1->diff_next/10000000;
				...
				{
					$temp_amt1 = $row1->diff_pre/100000;
					$temp_amt2 = $row1->diff_next/100000;
					echo "<td align='right'>".number_format($temp_amt1, 2)."</td>";
					echo "<td align='right'>".number_format($temp_amt2, 2)."</td>";
				...
				{
					echo "<td align='right'>".number_format($row1->diff_pre, 2)."</td>";
					echo "<td align='right'>".number_format($row1->diff_next, 2)."</td>";
				...
				//echo "<td align='right'>".number_format($row1->diff_next,2)."</td>";
				//echo "<td align='right'>".$row1->Diff."</td>";
				//echo "<td align='right'>".number_format($row1->diff_per,2)."</td>";//diff_per
			}
			echo "</tr>";
			$co++;
		}
		...
		foreach($iss_2_abn_incrdecr_data as $row1)
		{
			echo "<tr>";
				...
				if($row1->COA_ID_VALUE == 2)
				{
					...
					{
						echo "<td align='right'>".number_format(isset($row1->diff_pre)?$row1->diff_pre:'0', 0, '.', '')."</td>";
						echo "<td align='right'>".number_format($row1->diff_next, 0, '.', '')."</td>";
						echo "<td align='right'>"."0"."</td>";
						echo "<td align='right'>"."0"."</td>";
					}
	...
	echo form_open('iss/iss_abn_inr_dec_report_details/1', 'id="iss_item_2_query_form"');
    if(isset($previous_value) && !empty($previous_value))
    {
        foreach($previous_value as $key=>$val)
        {
   	    ...
   	    echo "<tr>";
        $attribute='style="background-color: #FF9900;" onclick="check_iss_form2_query_pdf(this.value)"';
