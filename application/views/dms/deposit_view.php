...
<p>
 ...
 <td align=center> </td>
	...
	<div id="error"><?php echo validation_errors(); ?></div>
  	...
  	<tr id="tbldesgTarget">
    <td ><strong>Up to the month From January to</strong></td>
    <td>&nbsp; 
    <!--<input type="text" name="upmonth" size="2" maxlength="2" value="<?php echo isset($_POST['upmonth'])?$_POST['upmonth']:'';?>"/>-->
    <?php 
        ...
        echo '<select name="upmonth" id="upmonth">';
        echo '<option value="">Select Month</option>';
    	foreach($month_array as $key=>$val)
    	{
    		$select='';
    		if(isset($_POST['upmonth']) && $_POST['upmonth']==$key)
    		{
    ...
    <td>&nbsp; 
    <!--<input type="text" name="year" size="4" maxlength="4" value="<?php echo isset($_POST['year'])?$_POST['year']:'';?>"/>-->
        <?php
        //$year_array=array('2013','2014','2015');
		$this->load->helper('base');
        $year_array=get_common_year(1);
        echo '<select name="year" id="year">';
        echo '<option value="">Select Year</option>';
    	foreach($year_array as $key=>$val)
    	{
    		$select='';
    		if(isset($_POST['year']) && $_POST['year']==$val)
    		{
    			$select="selected='selected'";
    		}
