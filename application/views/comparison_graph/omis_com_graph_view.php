    ...
    echo form_open('rpd/omis_com_graph_details','id="omis_search_form"');
    ...
    <td>
    <?php
        echo '<select name="report_of_date3" id="report_of_date3" >';
        echo '<option value="">Select Date</option>';
    	foreach($records3 as $row)
    	{
    		$select='';
    		if(isset($_POST['report_of_date3']) && $_POST['report_of_date3']==$row->om_dat_date)
    		{
    			$select="selected='selected'";
    		}
            echo '<option  value="'.$row->om_dat_date.'" '.$select.'>'.$row->om_dat_date.'</option>';
    ...
    <td COLSPAN="6"><input type="text" size="65px" name="search_text" id="search_text" value="" onkeyup="fetch_br_ao_do_omis_com_graph(this.value)"/></td>
    </tr>
