...
<div class="container">
   <div class="basic_info_content">
    <?php 
        $attributesForm = array('name' => 'lmsBasicEditForm', 'id'=>'lms_basic_search_os_form_view');
        echo form_open_multipart('lms/lms_os_entry_details_view', $attributesForm);
    ?>
    ...
    <table class="search_tbl">
        <tr>
            ...
            <td>
            <div class="form-group">
            <input type="button" class="btn btnSave" value="Search" <?php echo $attribute; ?> style="background-color: #E9967A; padding: 0px 18px;" onclick="lms_os_view_search(this.value)"/>            
            </div>
            </td>   
            <td></td>                 
        </tr>
    ...
    <table class="table table-bordered" border="1" id="mydata">
        ...
        <?php foreach($lmseditDatas as $lmseditData) { ?>
            <tr>
                <td><?php 
                    echo anchor('lms/lms_os_entry_details_view/'.$lmseditData->lb_tran_no, $lmseditData->lb_tran_no, 'title="Edit"');
                ?>
                </td>
