...
function check_search_form_iss2_0010(str)
 {
    if(str !='')
    {
      ...
      if(report_option_selector==1 || report_option_selector==5)
      {
        ...
        if(report_year !='')
        {
            ...
            if(str=='Mismatch Report'){report_click_btn=2;}
            jQuery('#report_click_btn').val(report_click_btn);
            jQuery('#iss_2_010_search_form').submit();
        }
        else
        {
	...
	<style>
   	...
   	<?php
    echo "<table align =center id=\"tbldesgTarget\"><tr><th>Integrated Supervision System(ISS) and OMIS Loan Related Items Cross Report</th></tr></table>";
    echo "</table>";

    echo form_open('iss/iss_2_010_report_details','id="iss_2_010_search_form"');
	echo '<div id="report_option_div" style="margin-top:50px;">';
    echo '<font style="color:brown;font-weight: bold;">ISS Form-2 Online-cl OMIS Loan Cross Report For: </font>';
    ...
    <table border="1" style="margin-top: 50px;display: none;" id="search_form_table_n">
		<tr>
			<th COLSPAN="4">
			 <h3 style="color: green;">FILL FORM FOR ISS FORM-2 CL CROSS REPORT</h3>
		</th>
		...
		<tr id="report_of_br_ao_do_div">
			<td COLSPAN="6">
			<input type="button" name="view_report" id="view_report" value="View Report" style="background-color: #FF9900;" onclick="check_search_form_iss2_0010(this.value)"/>
            <input type="button" name="missing_list" id="missing_list" value="Mismatch Report" style="background-color: #FF9900;" onclick="check_search_form_iss2_0010(this.value)"/>
			</td>
		</tr>		
	</table>
