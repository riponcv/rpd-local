...
function lms_os_edit_save(btn_click)
{
    if(btn_click =='Save Changes')
    {
        var outStandingE = document.lmsOsEditForm.osdAmtEN.value;
        var claimAmtEV = document.lmsOsEditForm.claimAmtEN.value;

        var outStandingErr = claimAmtErr = true;

        outStandingErr = validateTextSelect(outStandingE, 0, "oustatsndAmtE", "osAmtEEID", "outstanding", 0);
        claimAmtErr = validateTextSelect(claimAmtEV, 0, "claimAmtE", "claimAmtEEID", "claim amount", 0);

        if((outStandingErr || claimAmtErr) == true) {
            return false;
         } else {
            document.lmsOsEditForm.submit();
         }	
    }
}	
