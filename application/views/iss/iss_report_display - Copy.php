...
<p>
  ...
  if($this->session->flashdata('success_wp'))
   ...
   if(isset($data_exist) && count($data_exist)>1)
   {
		...
		foreach($records3 as $row3)
		{
			...
			if($row3->SUPERVISION_COA_ID == 1010310 && $row3->AMOUNT_BDT > 0 && (strlen($row3->AMOUNT_BDT) >= 8 || strlen($row3->AMOUNT_BDT)<11))
			{
				//if(strpos($row3->AMOUNT_BDT,'.')== true && strlen($row3->AMOUNT_BDT) == 11)
				{
					...
					//echo number_format($row3->AMOUNT_BDT,0,'.','')."<br/>";
				}
			}
	...
	Certificate Reporting: <?php echo isset($whole_certificate)?$whole_certificate:'0'; echo '/'; echo isset($whole_certificate)?$whole_certificate:'0'; ?></th></tr><?php } ?>
	<?php } ?>
	</table>
    <?php
	...
	if(isset($login_office_status) && $login_office_status != 4 && $login_office_status !=1)
	{
		...
		foreach($records1 as $row1)
		{
			...
			//if($login_office_status == 4){number_format($data_amt_total[$co], 0, '.', '')
				//echo "<td align='left'>".$ISLAMIC_CONVENTIONAL_INDICATOR."</td>";
			//}
			//else 
			{
	...
	if(isset($login_office_status) && $login_office_status ==1)
	{
		...
		foreach($records3 as $row_1)
		{
			...
			echo "<td align='left'>".$row_1->AMOUNT_BDT."</td>";
			
			echo "</tr>";
			$co++;
		}
	}
	//$co=0; $report_option_selector
	//if(isset($report_option_selector) && $report_option_selector ==1)
		...
		if(isset($login_office_status) && $login_office_status !=1)
		{
			...
			$concern_off = '';
			//Branch start here//
			//if(isset($report_option_selector_post) && $report_option_selector_post == 1 && isset($login_office_status_post) && $login_office_status_post==4)
			...
			//Area/Div/Whole end here//
			echo "<tr>";
			$attribute1 = 'style="background-color: #FF9900;width:200px;height:30px;font-size:16px;"';
			//echo "<td><input type='button' id='iss_cer_submit_btn' value='Save AS PDF' onclick='view_iss_form_pdf(this.value)'  $attribute1 /></td>";
			//echo "<td align='center' >".form_submit('actionbtn', 'Save AS PDF',$attribute)."</td>";
